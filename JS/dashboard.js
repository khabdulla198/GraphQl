...
async function loadProfileData() {
  try {
    ...
    if (user.progresses && user.progresses.length > 0) {
      const lastProgress = user.progresses[user.progresses.length - 1];
      if (lastProgress.object) {
        currentProjectName = lastProgress.object.name || "Unknown project";
      }
    }
...
function updateXpSection(boardEvents, currentProjectName) {
  ...
  boardEvents.forEach((ev) => {
    ...
    if (
      !existing ||
      new Date(ev.createdAt) > new Date(existing.createdAt)
    ) {
  ...
  const latestPerProject = Array.from(projectMap.values()).sort(
    (a, b) =>
      new Date(b.ev.createdAt) - new Date(a.ev.createdAt)
  );
